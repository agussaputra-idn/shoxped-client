...
// --- PARSER DATA (SAMA SEPERTI SEBELUMNYA) ---
...
// --- LOGIKA PENCARIAN CERDAS (SMART FILTER) ---
const smartFilter = (products: any[], query: string) => {
    if (!query) return products;

    // 1. Pecah Query User menjadi kata-kata (misal: "tas wanita" -> ["tas", "wanita"])
    const queryTerms = query.toLowerCase().trim().split(/\s+/);

    return products.filter(p => {
        const title = p.title.toLowerCase();

        // 2. Bersihkan Judul Produk:
        // Ganti simbol seperti /, -, (, ), koma, titik menjadi SPASI.
        // Tujuannya agar "Tas/Dompet" terbaca terpisah menjadi "tas" dan "dompet".
        const cleanTitle = title.replace(/[^a-z0-9]/g, ' ');
        
        // 3. Pecah Judul menjadi array kata
        // Contoh: "Atasan Wanita Murah" -> ["atasan", "wanita", "murah"]
        const titleWords = cleanTitle.split(/\s+/);

        // 4. CEK KECOCOKAN KATA (STRICT MATCH)
        // Kita cek apakah setiap kata yang diketik user ADA di dalam array kata judul.
        return queryTerms.every(term => {
            // Logika: Apakah ada kata di judul yang PERSIS sama dengan kata pencarian?
    ...
    <div className='w-full min-h-screen bg-gray-50 pb-12'>
      <div className='w-full max-w-[1920px] mx-auto px-4 md:px-8 pt-8'>
        <div className='flex items-center justify-between mb-6'>
            ...
            <span className="text-gray-500 text-sm">{products.length} Produk</span>
        </div>

        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6'>
            {loading ? (
                 [...Array(6)].map((_, i) => <div key={i} className='bg-white rounded-xl shadow-sm h-80 animate-pulse border border-gray-100' />)
            ) : products.length > 0 ? (
                products.map((item) => (
